...
- Context API is used when there is global data the is shared by multiple components. The API helps you keep your state clean.
2. In your own words, describe `actions`, `reducers` and the `store` and their role in Redux. What does each piece do? Why is the store known as a 'single source of truth' in a redux application?
- Actions are the source of information for the store.Reducer is a function that finalizes changes to the app state and uses action creators to find out the time of the specific change. Store is a single javascript object, it changes and holds state for app
3. What does `redux-thunk` allow us to do? How does it change our `action-creators`?
- redux thunk is a middleware that lets you call action creators that return a function instead of an action object. that function recieves the store's dispatch method, which is then used to dispatch regulare
synchronous actions inside the functions body. changes our action creators with thunk.
4. What is your favorite state management system you've learned and this sprint? Please explain why!
i enjoyed context API the most out of them all because of how much more simpler and easier it is to understand compared to the others.
